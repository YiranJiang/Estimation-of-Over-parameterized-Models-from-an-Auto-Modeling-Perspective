...
def validate(model, device, valid_loader):
    ...
    with torch.no_grad():
        for data, target in valid_loader:
            ...
            output = model(data)
            valid_loss += criterion_sum(output, target).item()  # sum up batch loss
    ...
    return([valid_loss,100. * correct / len(valid_loader.dataset)])    
...
def train_estimation(args, model, device, batch_size, full_data, full_label, current_indices, future_indices, imp_prob, all_label, train_full_loader, optimizer, epoch,
         am = True, l1_pen = True, decay = 0.99, warm_up_num_epoch = 50, l1_pen_val = 1e-5, l2_pen_val = 1e-4, isWeighted = True, isVerbose = True):
    model.train()
...
if __name__ == '__main__':
    ...
    train_dataset, val_dataset = random_split(dataset1, [50000, 10000])

    train_full_loader = torch.utils.data.DataLoader(train_dataset, batch_size=64, shuffle=True)
    ...
    array_1 = np.array([*range(len(full_label))])
    patience = 5
    matrix = np.empty((4, 5))
    lst = [FC_Net1(), FC_Net2(), FC_Net3(), CNN_Net()]
    matrix = np.empty((4, 5))

    for i in range(len(lst)):
        model = lst[i].to(device)
        torch.save(model.state_dict(), 'null_model.pt') 
        ...
        for NN in range(5):    
            ...
            while True:
                train_loss = train_estimation(args, model, device, args.batch_size,full_data,full_label, array_1, array_1, full_label, full_label, train_full_loader,optimizer, epoch,
                      am = False, l1_pen = True, decay = 1, l1_pen_val=0, l2_pen_val=0, isWeighted = False, isVerbose = False)           

                return_list = validate(model, device, valid_loader)

                scheduler.step()
                if best_loss is None or return_list[0] < best_loss:
                    best_loss = return_list[0]
            ...
            matrix[i, NN] = 100 - return_list[1]

    row_means = np.mean(matrix, axis=1)
    row_stds = np.std(matrix, axis=1)

    print("FC-400, FC_800, FC_1600, CNN")
