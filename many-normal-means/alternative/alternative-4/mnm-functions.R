...
d_eta_alpha_w <- function(etas,alphas,ys,ys_imp,w){
  n1 <- length(ys)
  n2 <- length(ys_imp)
  ...
  for(i in 1:n1){
    ...
    for (k in 1:m){
      d_etas_1[k] <- d_etas_1[k] + (alphas[k] * exp(- (1/2)*((ys[i] - etas[k])^2)) * (ys[i] - etas[k]))/this.sum
      
      d_alphas_1[k] <- d_alphas_1[k] + exp(- (1/2)*((ys[i] - etas[k])^2))/this.sum
    }
  }
  
  
  d_etas_1 <- d_etas_1 / n1
  d_alphas_1 <- d_alphas_1 / n1
  ...
  for(i in 1:n2){
    this.sum <- 0
    for (k in 1:m){
      this.sum <- this.sum + alphas[k] * exp(- (1/2)*((ys_imp[i] - etas[k])^2))
    }
    
    for (k in 1:m){
      d_etas_2[k] <- d_etas_2[k] + (alphas[k] * exp(- (1/2)*((ys_imp[i] - etas[k])^2)) * (ys_imp[i] - etas[k]))/this.sum
      
      d_alphas_2[k] <- d_alphas_2[k] + exp(- (1/2)*((ys_imp[i] - etas[k])^2))/this.sum
    }
  }
  
  
  d_etas_2 <- d_etas_2 / n2
  d_alphas_2 <- d_alphas_2 / n2
  
  return(-c(w*d_etas_1 + (1-w)*d_etas_2, w*d_alphas_1 + (1-w)*d_alphas_2))
}
...
param_update_w <- function(lambdas,ys, ys_imp, w = 0.5, max_iteration = 500, tol = 1e-6,
                           step.size = 0.05, start_etas = 0, start_alphas = 0){
  ...
  for(ii in 1:max_iteration){
    ...
    hist <- c(hist, w*mynll(eta0s, alpha0s, ys) + (1-w)*mynll(eta0s, alpha0s, ys_imp))
    
    this.g <- d_eta_alpha_w(eta0s,alpha0s,ys,ys_imp,w)
    
    new.etas <- eta0s - 10*step.size* (this.g[1:m])
...
mnm_AM <- function(ys,m,BB = 5, K = 5, isBootstrap = T, J = 2000, max_iteration = 500, tol = 1e-6,
  ...
  # 
  # 
  # result.list[[1]] <- param_update(lambdas0,ys,max_iteration, tol,
  #                                   step.size)
  #
   result.list <- list()
  ...
  ys_imp_1 <- ys_imp[1:(length(ys_imp)/2)] + rnorm(length(ys_imp)/2, mean = 0, sd = 0.1)
  ys_imp_2 <- ys_imp[((length(ys_imp)/2)+1):length(ys_imp)] 
  future.nll.vec <- rep(0, length(ws))
    
  for (i in 1:length(ws)){
  
    result.list[[i]] <- param_update_w(lambdas0,ys,ys_imp_1, ws[i], max_iteration, tol,
                           step.size)
    
    future.nll.vec[i] <- mynll(result.list[[i]]$etas_hat, result.list[[i]]$alphas_hat, ys_imp_2)

  }
  
  cat("Selected w: ", ws[which.min(future.nll.vec)], "\n")
  
  
  
  return(result.list[[which.min(future.nll.vec)]])
}
...
eva_dpmm <- function(ys,mus) {
  
  mus_hat <- rep(NA,length(ys))


  # dp <- DirichletProcessGaussian(ys)
